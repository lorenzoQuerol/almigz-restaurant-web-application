...
const PrivDialog = ({ openPrivDialog, handleOpenPrivDialog }) => {
	const { data, error } = useSWR("/api/privacyPolicy", fetcher);
	
	if (!data) return <h1 className="overlay">Loading...</h1>;

	const sections = data.privacyItems.sort((a, b) => a.sectionNumber - b.sectionNumber);
	...
	const options = {
	  ...
	  return (
		<>
			<Transition appear show={openPrivDialog} as={Fragment}>
				<Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto text-slate-900" onClose={handleOpenPrivDialog}>
						...
						>
							<div className="inline-block w-full max-w-2xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
								...
								{sections.map((section) => (
									<div className="pb-2">
										<h1 className="pb-2 font-semibold">{section.sectionNumber}.0<span className="pl-2">{section.sectionTitle}</span></h1>
